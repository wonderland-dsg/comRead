...
IplImage* drawFloatGraph(CvScalar colorGraph,const float *arraySrc, int nArrayLength, IplImage *imageDst, float minV, float maxV, int width, int height, char *graphLabel, bool showScale)
{
	int w = width;
	int h = height;
	...
	for (int i=0; i<nArrayLength; i++) {
		...
		cvLine(imageGraph, ptPrev, ptNew, colorGraph, 0, CV_AA);	// Draw a line from the previous point to the new point
		ptPrev = ptNew;
	}
...
void showFloatGraph(CvScalar colorGraph,const char *name, const float *arraySrc, int nArrayLength, int delay_ms, IplImage *background)
{
#ifdef USE_HIGHGUI
	// Draw the graph
	IplImage *imageGraph = drawFloatGraph(colorGraph,arraySrc, nArrayLength, background);
